...
public class SendFileViaSFTP {
	...
	public void SendFile(String filePath) {
		...
		try {
			...
			// System.out.println("Changed the directory to
			// "+channelSftp.pwd());
			...
			/*
			 * Vector lsResult =channelSftp.ls(SFTPWORKINGDIR); for (int i=0;
			 * i<lsResult.size(); i++){ System.out.println(lsResult.get(i)); }
			 */
			...
			if (e.getMessage().toString().contains("java.net.UnknownHostException:")) {
				...
				MainWindow.showErrorMessage(
						"Connection with " + this.SFTPHOST + " was interrapted \n " + e.getMessage().toString());
			} else {
				MainWindow.showErrorMessage(e.getMessage().toString());
			}
